...
import com.t3.client.ui.zone.vbl.AreaOcean;
...
*/
	...
	public static void exposePCArea(ZoneRenderer renderer) {
		...
		boolean isGM = TabletopTool.getPlayer().getRole() == Role.GM;
		...
		for (Token token : tokList) {
			...
			if ((!TabletopTool.isPersonalServer() || TabletopTool.getServerPolicy().isUseIndividualViews()) && !owner) {
				continue;
			}
...
//			zone.exposeArea(visionArea, filteredToks);
			...
			zone.putToken(token);
			TabletopTool.serverCommand().exposeFoW(zone.getId(), visionArea, filteredToks);
			TabletopTool.serverCommand().updateExposedAreaMeta(zone.getId(), exposedGUID, meta);
		}
	}
