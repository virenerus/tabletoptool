...
/**
 * <p>
 * This class acts as the basis for various TabletopTool subsystems to store data in
 * an external file.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * The TabletopTool object being stored should have a {@code toString()} method that
...
abstract public class DataTemplate {
	...
	protected final void addDataObjects(String path, Map<String, ? extends Object> data) {
		...
		for (int i = 0; i < sorted.length; i++) {
			...
			model.addNode(path, new T3Node(sorted[i], one));
		}
	}
	 ...
	 * <code>List</code>) and adds a <code>T3Node</code> for
	 * each one to the model.  The keys are sorted alphabetically before being added to
	 * the list.
	 * </p>
	 * @param path the <code>String</code> representation of the TreePath
