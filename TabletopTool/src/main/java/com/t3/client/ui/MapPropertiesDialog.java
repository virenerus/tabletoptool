...
import com.t3.model.grid.GridlessGrid;
import com.t3.model.grid.HexGridHorizontal;
import com.t3.model.grid.HexGridVertical;
...
public class MapPropertiesDialog extends JDialog {
	...
	private void initialize() {
		...
		formPanel = new FormPanel("com/t3/client/ui/forms/mapPropertiesDialog.xml");
		...
		TextureChooserPanel textureChooserPanel = new TextureChooserPanel(paintChooser, TabletopTool.getFrame().getAssetPanel().getModel(), "mapPropertiesTextureChooser");
	...
	private void accept() {
		...
		T3Util.uploadTexture(backgroundPaint);
		T3Util.uploadTexture(fogPaint);
		if (mapAsset != null) {
			AssetManager.putAsset(mapAsset);
			if (!TabletopTool.isHostingServer()) {
				TabletopTool.serverCommand().putAsset(mapAsset);
			}
		}
		copyUIToZone();
		status = Status.OK;
	...
	private void copyUIToZone() {
		...
		zone.setUnitsPerCell(StringUtil.parseInteger(getDistanceTextField().getText(), zone.getUnitsPerCell()));
		zone.setGrid(createZoneGrid());
		zone.setTokenVisionDistance(StringUtil.parseInteger(getDefaultVisionTextField().getText(), zone.getTokenVisionDistance()));
...
//							setBackgroundAsset(asset, getImageFileChooser().getSelectedThumbnailImage());
							...
							TabletopTool.showError("Could not load that map: " + ioe);
							selectedAsset = null;
						}
					}
				}
