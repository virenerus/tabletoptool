...
public class InitiativeListModel extends AbstractListModel<TokenInitiative> implements PropertyChangeListener {
    ...
    public int getDisplayIndex(int index) {
        if (index < 0 || TabletopTool.getFrame().getInitiativePanel().hasGMPermission())
            return index;
    ...
    public static boolean isTokenVisible(Token token, boolean hideNPC) {
        ...
        if (TabletopTool.getFrame().getInitiativePanel().hasGMPermission()) return true;
	...
	public int getSize() {
        ...
        if (TabletopTool.getFrame() == null || TabletopTool.getFrame().getInitiativePanel().hasGMPermission())
          return list.getSize();
        int size = 0;
        for (int i = 0; i < list.getSize(); i++) {
            TokenInitiative ti = list.getTokenInitiative(i);
